			...
			for step in xrange(self.step_per_episode):
				...
				if self.replay_buffer.get_size() > self.args.training_start:
					...
					if self.args.bn:
						...
						action_gradient = self.sess.run(self.critic_network.gradients, feed_dict={self.critic_network.states:batch_s, self.critic_network.actions:action_batch_q_gradient}) 
				...
				if done:
					...
					utils.write_logs(self.steps, self.reward_per_episode, episode+1, start_time, mode='train', total_loss=self.cost_per_episode)
